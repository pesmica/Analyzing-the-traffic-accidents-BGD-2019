...
# df$Outcome[df$Outcome=="Sa mat.stetom"]=="Material_damage"
# df$Outcome[df$Outcome=="Sa povredjenim"]=="Injury"
# df$Outcome[df$Outcome=="Sa poginulim"]=="Death"
```
...
ggplot(data = df_f_month, 
  ...
  theme_bw()
```
#There are no difference between frequencies of accidents. 
...
#Now, we will make three periods per months, I period: 1-10., II period: 11-20., III period:21-30
#Period I
```{r,echo=TRUE}
df_f$period<-factor(ifelse(df_f$Day <= 10, 1, ifelse(df_f$Day>=11 & df_f$Day <= 20, 2, 3)))

df_f_month_period<-df_f %>%
  filter(period==1) %>%
  group_by(Month, y) %>%
  summarise(n = n()) %>%
  mutate(Percent = round(n/sum(n)*100,2))
...
m$p_value<-Xsq$p.value
m %>% 
  kable(booktabs = T,longtable = TRUE, caption = "Accidents during January and February from 1st until 10th") %>%
  kable_styling(latex_options = c("hold_position", "repeat_header")) %>%
  collapse_rows(columns = 4)

ggplot(data = df_f_month_period, 
       aes(x = Month, y = Percent, color = y, fill = y)) +
  geom_bar(stat = "identity", position = "dodge", width = .5) + 
  scale_fill_brewer(palette = "Accent") +
  scale_color_brewer(palette = "Accent") +
...
m$p_value<-Xsq$p.value
m %>% 
  kable(booktabs = T,longtable = TRUE, caption = "Accidents during January and February from 21st until the end of the month") %>%
  kable_styling(latex_options = c("hold_position", "repeat_header")) %>%
  collapse_rows(columns = 4)
...
ggplot(data = df_f_month_period, 
  ...
  theme_bw()
```
#Here we have got bordered statisfical significan difference! So, we can ...

#Let's see now is any difference between hours per period
#Period I
```{r,echo=TRUE}
df_f_month_period_hour<-df_f %>%
  filter(period==1) %>%
  group_by(Month, Hour, y) %>%
  summarise(n = n()) %>%
  mutate(Percent = round(n/sum(n)*100,2))

#is any hour without accident? NO
length(which(df_f_month_period_hour$n == 0))


ggplot(data = df_f_month_period_hour, 
       aes(x = Hour, y = n, color = Month, fill = y)) +
  geom_bar(stat = "identity", position = "dodge", width = .5) + 
  scale_fill_brewer(palette = "Accent") +
  scale_color_brewer(palette = "Accent") +
  ggtitle("Period I")+
...
```
We have overdispersed count data. We need to select apropriate model: negative 
download wheather for Belgrade 01.01.2019-28.02.2019.
...
df_f_period<-df_big %>%
  filter(period==1)%>%
  group_by(Month, Hour, y) %>%
  summarise(Wind_speed = mean(Wind.Speed...10.m.above.gnd.),n = n()) %>%
  mutate(Percent = round(n/sum(n)*100,2))

plot(df_f_period$y)
table(df_f_period$y)

#is any hour without accident in Period I? NO
length(which(df_f_period$n == 0))
#how many accidents with death outcome? Yes, 2.
length(which(df_f_period$y == "Death"))

ggplot(data = df_f_period, 
       aes(x = Hour, y = n, color = Month, fill = y)) +
...
ggplot(data = df_f_period, 
       aes(x = Hour, y = Wind_speed, color = Month, fill = y)) +
  geom_bar(stat = "identity", position = "stack", width = .5) + 
  scale_fill_brewer(palette = "Accent") +
  scale_color_brewer(palette = "Set1") +
  ggtitle("Period I")+
...
with(df_f_period, tapply(n, y, function(x) {
    sprintf("M (SD) = %1.2f (%1.2f)", mean(x, na.rm = T), var(x,na.rm = T))
}))

ggplot(df_f_period, aes(n, fill = y)) + geom_histogram(binwidth = 1) + facet_grid(y ~ 
    ., margins = TRUE, scales = "free")

df_f_period$y<-as.character(df_f_period$y)
fit1 <- glm.nb(n ~ y+Wind_speed, data = df_f_period, link = log)
coef(fit1, matrix = TRUE)
summary(fit1)

# poisson <- glm(n ~ y+T_mean+Wind_speed, data = df_f_period, family = "poisson")
# pchisq(2 * (logLik(fit) - logLik(poisson)), df = 1, lower.tail = FALSE)

(est <- cbind(Estimate = coef(fit1), confint(fit1)))
exp(est)

newdf <- data.frame(Wind_speed = mean(df_f_period$Wind_speed), y = factor(1:3, levels = 1:3, 
    labels = levels(as.factor(df_f_period$y))))
newdf$yhat <- predict(fit1, newdf, type = "response")


newdf2 <- data.frame(
  Wind_speed = rep(seq(from = min(df_f_period$Wind_speed), to = max(df_f_period$Wind_speed), length.out = 100), 3),
  y = factor(rep(1:3, each = 100), levels = 1:3, labels =
  levels(as.factor(df_f_period$y))))

newdf2 <- cbind(newdf2, predict(fit1, newdf2, type = "link", se.fit=TRUE))
